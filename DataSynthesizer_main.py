...
class DataSynthesizerApp:
    """
    A Streamlit application for generating synthetic data using DataSynthesizer.
    """
    ...
    def __init__(self):
        ...
        self.description_file = TEMP_DIR / "description.json"
        self.synthetic_data_file = TEMP_DIR / "synthetic_data.csv"
    ...
    def render_sidebar(self) -> None:
        ...
        st.sidebar.info(
            """
            This application uses DataSynthesizer to generate synthetic data 
    ...
    def upload_and_process_data(self) -> None:
    ...
    def configure_column_types(self) -> None:
        """Allow users to configure column types and candidate keys."""
        st.subheader("Configure Column Types")
        
        with st.expander("Column Configuration", expanded=True):
            st.info("Select categorical columns and candidate keys (unique identifiers)")
            ...
            st.write("Candidate Keys (Unique Identifiers)")
    ...
    def generate_synthetic_data(self) -> bool:
        ...
        with st.spinner("Generating synthetic data..."):
            try:
                ...
                self.attribute_description = read_json_file(str(self.description_file))['attribute_description']
                
                elapsed_time = time.time() - start_time
                st.success(f"Successfully generated {self.num_tuples} synthetic records in {elapsed_time:.2f} seconds")
    ...
    def display_results(self) -> None:
        """Display and analyze the synthetic data results."""
        if self.synthetic_df is None:
            return
        
        st.header("Synthetic Data Results")
        
        # Create tabs for different views
        tab1, tab2, tab3, tab4 = st.tabs(["Synthetic Data", "Comparison", "Visualizations", "Download"])
        ...
        with tab2:
            ...
            st.write("Select columns to compare distributions:")
            ...
            if "Correlated" in self.mode:
                ...
                try:
                    inspector = ModelInspector(self.input_df, self.synthetic_df, self.attribute_description)
                    ...
                    st.write("This heatmap shows how well the correlations between attributes are preserved in the synthetic data.")
                except Exception as e:
        ...
        with tab3:
            ...
            if viz_type == "Pair Plot" and len(numeric_cols) >= 2:
                ...
                if len(pair_cols) >= 2:
                    st.write("Original Data Pair Plot")
                    fig1 = sns.pairplot(self.input_df[pair_cols])
                    st.pyplot(fig1)
                    
                    st.write("Synthetic Data Pair Plot")
                ...
                ax[0].set_title(f"Original - {box_col}")
                
                # Synthetic data box plot
                sns.boxplot(y=self.synthetic_df[box_col], ax=ax[1])
                ax[1].set_title(f"Synthetic - {box_col}")
        ...
        with tab4:
            ...
            st.download_button(
                label="Download Data Description (JSON)",
                data=json_data,
                file_name="data_description.json",
                mime="application/json"
            )
            
            # Option to download a sample analysis report
            if st.button("Generate Analysis Report"):
                report_html = self.generate_analysis_report()
                st.download_button(
                    label="Download Analysis Report",
                    data=report_html,
                    file_name="synthetic_data_analysis.html",
                    mime="text/html"
                )
    
    def generate_analysis_report(self) -> str:
        """Generate an HTML report comparing original and synthetic data."""
        buffer = io.StringIO()
        ...
        <html>
        <head>
            <title>Synthetic Data Analysis Report</title>
            <style>
        ...
        <body>
            <div class="container">
                <h1>Synthetic Data Analysis Report</h1>
                ...
                <h2>Column Statistics Comparison</h2>
        """)
        
        # Add statistics for each column
        for col in self.input_df.columns:
            buffer.write(f"<h3>Column: {col}</h3>")
            ...
            if col in self.input_df.select_dtypes(include=['number']).columns:
                ...
                buffer.write("<table><tr><th>Statistic</th><th>Original</th><th>Synthetic</th></tr>")
                ...
                buffer.write("<table><tr><th>Value</th><th>Original Count</th><th>Original %</th><th>Synthetic Count</th><th>Synthetic %</th></tr>")
        ...
        buffer.write("""
                <div class="footer">
                    <p>Generated using DataSynthesizer - https://github.com/DataResponsibly/DataSynthesizer</p>
                </div>
            </div>
        </body>
        </html>
    ...
    def run(self) -> None:
        ...
        if self.upload_and_process_data():
            if st.button("Generate Synthetic Data", type="primary"):
                if self.generate_synthetic_data():
                    self.display_results()

if __name__ == "__main__":
    app = DataSynthesizerApp()
    app.run()
